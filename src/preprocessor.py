    ...
    def _load_records(self):
        for i in self.metadata.index:
            ...
            if not os.path.exists(fpath):
                ...
                try:
                    records = pd.read_csv(fpath, header=0, encoding='utf-8-sig')
                except UnicodeDecodeError:
                    records = pd.read_csv(fpath, header=0, encoding='cp949')
            else:
                records = pd.read_pickle(fpath)
